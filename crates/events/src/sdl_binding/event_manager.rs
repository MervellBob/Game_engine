...
impl EventManager {
        ...
        for event in event_pump.poll_iter() {
            match event {
                ...
                SdlEvent::Quit { timestamp } => {
                    let app_event: AppEvent = AppEvent::Quit { timestamp };
                    self.app_events_queue.push(app_event)
                }
                SdlEvent::AppTerminating { timestamp } => {
                    let app_event: AppEvent = AppEvent::AppTerminating { timestamp };
                    self.app_events_queue.push(app_event)
                }
                SdlEvent::AppLowMemory { timestamp } => {
                    let app_event: AppEvent = AppEvent::AppLowMemory { timestamp };
                    self.app_events_queue.push(app_event)
                }
                SdlEvent::AppWillEnterBackground { timestamp } => {
                    let app_event: AppEvent = AppEvent::AppWillEnterBackground { timestamp };
                    self.app_events_queue.push(app_event)
                }
                SdlEvent::AppDidEnterBackground { timestamp } => {
                    let app_event: AppEvent = AppEvent::AppDidEnterBackground { timestamp };
                    self.app_events_queue.push(app_event)
                }
                SdlEvent::AppWillEnterForeground { timestamp } => {
                    let app_event: AppEvent = AppEvent::AppWillEnterForeground { timestamp };
                    self.app_events_queue.push(app_event)
                }
                SdlEvent::AppDidEnterForeground { timestamp } => {
                    let app_event: AppEvent = AppEvent::AppDidEnterForeground { timestamp };
                    self.app_events_queue.push(app_event)
                }
                ...
                SdlEvent::MouseButtonDown {
                    ...
                    let input_event: InputEvent = InputEvent::MouseButtonDown {
                        ...
                        window_id,
                        which,
                        mouse_button: MouseButton::try_from(mouse_btn as u8)
    ...
    fn map_sdl_mousewheel_direction(
        &self,
    ...
    fn map_sdl_keycode(
        &self,
        sdl_keycode: Option<sdl3::keyboard::Keycode>,
        timestamp: u64,
    ) -> Keycode {
        match sdl_keycode {
    ...
    fn map_sdl_scancode(
        &self,
        sdl_scancode: Option<sdl3::keyboard::Scancode>,
        timestamp: u64,
    ) -> Scancode {
        match sdl_scancode {
...
mod tests {
    ...
    fn map_sdl_scancode_valid_code() {
        ...
        assert_eq!(
            manager.map_sdl_scancode(Some(SdlScancode::A), 123),
            Scancode::A,
            "Mapping of scancode A failed"
        );
        assert_eq!(
            manager.map_sdl_scancode(Some(SdlScancode::CapsLock), 456),
            Scancode::CapsLock,
            "Mapping of scancode CapsLock failed"
        );
    }
    #[test]
    fn map_sdl_scancode_none() {
        let manager = EventManager::new();
        assert_eq!(
            manager.map_sdl_scancode(None, 111),
            Scancode::Unknown,
            "Mapping of scancode 'None' failed"
        );
    }
    ...
    fn map_sdl_keycode_valid_code() {
        let manager = EventManager::new();
        assert_eq!(
            manager.map_sdl_keycode(Some(SdlKeycode::A), 123),
            Keycode::A,
            "Mapping of Keycode A failed"
        );
        assert_eq!(
            manager.map_sdl_keycode(Some(SdlKeycode::CapsLock), 456),
            Keycode::CapsLock,
            "Mapping of Keycode CapsLock failed"
        );
    }
    #[test]
    fn map_sdl_keycode_none() {
        let manager = EventManager::new();
        assert_eq!(
            manager.map_sdl_keycode(None, 111),
            Keycode::Unknown,
            "Mapping of Keycode 'None' failed"
        );
    }
    ...
    fn map_sdl_mousewheel_direction_normal() {
        let manager = EventManager::new();
        assert_eq!(
            manager.map_sdl_mousewheel_direction(SdlMouseDirection::Normal),
            MouseWheelDirection::Normal,
            "Mapping of mousewheel direction normal failed"
        );
    }
    #[test]
    fn map_sdl_mousewheel_direction_flipped() {
        let manager = EventManager::new();
        assert_eq!(
            manager.map_sdl_mousewheel_direction(SdlMouseDirection::Flipped),
            MouseWheelDirection::Flipped,
            "Mapping of mousewheel direction flipped failed"
        );
    }
    #[test]
    fn map_sdl_mousewheel_direction_unknown() {
        let manager = EventManager::new();
        assert_eq!(
            manager.map_sdl_mousewheel_direction(SdlMouseDirection::Unknown(12)),
            MouseWheelDirection::Unknown,
            "Mapping of mousewheel direction unknown failed"
        );
    }
}
